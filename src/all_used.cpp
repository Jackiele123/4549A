...
void InchToCoordConversion(float startX, float startY, float orientation, float inchesToTravel) {

	float dx = inchesToTravel * sin(orientation * 3.14 / 180);
	float dy = inchesToTravel * cos(orientation * 3.14 / 180);

	float finalX = startX + dx;
	float finalY = startY + dy;
}
